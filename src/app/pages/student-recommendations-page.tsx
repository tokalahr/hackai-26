...
type StudentRecommendationsPageProps = {
  onNavigate: (path: string) => void;
};

export function StudentRecommendationsPage({ onNavigate }: StudentRecommendationsPageProps) {
  const input = readLearningInput();

  if (!input || input.role !== "student") {
    return (
      <section className="relative min-h-[70vh] pb-16 overflow-hidden">
        <div className="absolute inset-0 bg-[#0D0D0F]" />
        <div className="relative z-10 space-y-5">
          <h1 className="text-4xl font-bold text-white">Student Recommendations</h1>
          <p className="text-gray-400">No student input found. Please submit the Learning Assistant form first.</p>
          <button
  ...
  const rec = buildRoleRecommendation(input, "student");
    ...
    <section className="relative min-h-[70vh] pb-16 overflow-hidden">
      ...
      <div className="relative z-10 space-y-6">
        ...
        <p className="text-gray-400 max-w-2xl">
          Tailored for coursework planning and academic progress for {input.name || "the learner"}.
        </p>

        <div className="bg-white/5 border border-white/10 rounded-2xl p-5 space-y-4">
          <p className="text-[#7bf0f0] font-semibold">{rec.headline}</p>
          ...
          <div>
            <p className="text-sm font-semibold text-gray-300 mb-2">Coursework focus</p>
            <div className="flex flex-wrap gap-2">
              {rec.focusAreas.map((focus) => (
                <span key={focus} className="px-3 py-1 rounded-full text-xs bg-[#00A8A8]/15 border border-[#00A8A8]/25 text-[#7bf0f0]">
                  {focus}
                </span>
              ))}
            </div>
