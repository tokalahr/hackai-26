...
func ConnectDB() *mongo.Client {
	once.Do(func() {
		...
		log.Printf("Attempting to connect to MongoDB with URI: %s", GetEnvMongoURI())
...
func ConnectClubsDB() *sql.DB {
	clubOnce.Do(func() {
		...
		dbURI := GetClubsDBUri()
		log.Printf("Attempting to connect to Clubs DB with URI: %s", dbURI)
		...
		log.Printf("Successfully connected to Clubs DB")
		clubsDbInstance = db
	})
...
func InsertData(data interface{}) {
	log.Printf("Inserting data: %+v", data)
	// Add logic to insert data into the database
}

func InsertTestData() {
	collection := GetCollection("testCollection")
	_, err := collection.InsertOne(context.Background(), bson.M{"name": "Test", "value": 123})
	if err != nil {
